...
export default function SpecialList() {
  ...
  function buttonHandle(item) {
    try {
      ...
      Alert.alert("Added " + item.name + " To Your Order");
    } catch (e) {
      Alert.alert(e);
    }
  }
  useEffect(() => {
    axios
      .get(BASEURL + "api/menu-items/specials")
      .then((res) => {
  ...
  return (
    <Card>
      <Card.Title style={styles.cardTitles}>Specials</Card.Title>
      <Card.Divider />
      {menuItems.map((card) => {
        return (
          <View key={card.id} style={styles.container}>
            ...
            <Text style={styles.container}>{card.description}</Text>
